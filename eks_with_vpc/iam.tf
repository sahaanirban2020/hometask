resource "aws_iam_role" "cluster" {
    name = format("%s-%s", lookup(var.vpc, "name"), "eksCluster")
    path = "/"
...
resource "aws_iam_policy" "eksSvcLnkdRolePolicy" {
    name        = format("%s-%s", lookup(var.vpc, "name"), "EKSServiceLinkedRolePolicy")
    path        = "/"
...
resource "aws_iam_role" "workerNodes" {
    name = format("%s-%s", lookup(var.vpc, "name"), "workerNodes")
    path = "/"
    assume_role_policy = <<EOF
{
    "Version": "2012-10-17",
...
resource "aws_iam_policy" "EKSALBIngressPolicy" {
    name        = format("%s-%s", lookup(var.vpc, "name"), "EKSALBIngressPolicy")
    path        = "/"
